...
public class TopicContentDataMapper implements RowMapper<TopicDataModel> {
	...
	public TopicDataModel mapRow(ResultSet rs, int rowNum) throws SQLException {
		...
		topicDataModel.setEntities(rs.getString("PersonEntities")+"|" +rs.getString("OrganizationEntities")+"|" +rs.getString("MoneyEntities"));
		...
		if (difference_In_Days <= 15 && topicDataModel.getRelevancy() > .90 && topicDataModel.getPopularity() > 16) {
			topicDataModel.setRating(5);

		} else if (difference_In_Days <= 30 && topicDataModel.getRelevancy() > .75
				&& topicDataModel.getPopularity() > 50) {
			topicDataModel.setRating(4);

		} else if (difference_In_Days <= 30 && topicDataModel.getRelevancy() > .90
				&& topicDataModel.getPopularity() > 50) {
			topicDataModel.setRating(4);
